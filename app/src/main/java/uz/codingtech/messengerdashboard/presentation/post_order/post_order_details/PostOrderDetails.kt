...
import uz.codingtech.messengerdashboard.presentation.common.MediaPreview
...
fun PostOrderDetails(
    ...
    orderId: String,
    orderViewModel: PostOrderDetailsViewModel = hiltViewModel()
) {
    ...
    if (showCancelDialog) {
        AlertDialog(
            ...
            confirmButton = {
                TextButton(
                    onClick = {
                        orderViewModel.event(PostOrderDetailEvent.Cancel(orderId))
    ...
    LaunchedEffect(Unit) {
        ...
        orderViewModel.event(PostOrderDetailEvent.InitOrderDetail(orderId))
    }
                ...
                Card(
                    ...
                    Card(
                        ...
                        Column(
                            ...
                            val media_uri = state.success?.media_url
                            if (media_uri?.isNotEmpty() == true) {
                                val safeLink = media_uri.replace("http://", "https://")
                                MediaPreview(link = safeLink)
                            }
                            Text(
                                text = "text: ${state.success?.text}",
                                modifier = Modifier.fillMaxWidth()
                            )
                            Text("link: ${state.success?.link}")
                            Text("created order: ${formatDateTime(state.success?.created_at)}")
                            Text("cpm: ${state.success?.spm}")
                            Text("budget: ${state.success?.budget}")

                            if (state.success!!.channels.isNotEmpty()) {
                                ...
                                FlowRow(
                                    ...
                                    var channels = state.success!!.channels.split(',')
                    ...
                    PlayPauseButton(
                        ...
                        onToggle = {
                            orderViewModel.event(
                                PostOrderDetailEvent.ChangeActive(
                                    orderId,
