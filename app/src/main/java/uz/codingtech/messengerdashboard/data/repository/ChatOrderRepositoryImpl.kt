...
@Singleton
class ChatOrderRepositoryImpl @Inject constructor(
    private val tokenDataStore: TokenDataStore,
    private val apiService: ApiService,
) : ChatOrderRepository {

    override suspend fun getOrders(page: Int): UiResult<ChatOrderPageModel> {
    ...
    override suspend fun postOrder(postOrderModel: PostOrderChatModel): UiResult<String> {
        val token = tokenDataStore.authData.first()?.access
        if (token == null) {
            return UiResult.Error(ErrorType.UnAuthorized)
        }
        return try {
            val response = apiService.postChatOrder("Bearer $token", postOrderModel)
            if (response.isSuccessful) {
    ...
    override suspend fun getOrderById(id: Int): UiResult<ChatOrderModel>{
        val token = tokenDataStore.authData.first()?.access
        if (token == null) {
            return UiResult.Error(ErrorType.UnAuthorized)
        }
